...
cstr e_strfsec(cstr dest, int dlen, constr dfmt, time_t sec)
{
    ...
    return localtime_r(&sec, &_tm) ? strftime(dest, dlen, dfmt, &_tm) ? dest
                                                                      : 0
                                   : 0;
}
...
i64 e_elapsepsec(constr from, constr ffmt)
{
    i64 elapse_sec = __elapsesecffmt(from, ffmt);
    ...
    if(elapse_sec == -1)
    {
        ...
        {
            _tm.tm_year += 1970;
            elapse_sec  = mktime((struct tm* const)&_tm) + _tm.tm_gmtoff;
        }
    }
...
i64 e_elapsepms(constr from, constr ffmt)
{
    i64 elapse_sec = __elapsesecffmt(from, ffmt);
